#   PongAIvAI
#   Credits: Authors: Michael Guerzhoy and Denis Begun
    ...
    def move(self, paddles, table_size, move_factor):
        ...
        for paddle in paddles:
            if self.frect.intersect(paddle.frect):
                ...
                if v[0] * (
                        2 * paddle.facing - 1) < 1:  # ball is not traveling (a) away from paddle (b) at a sufficient
                    # speed
                    v[1] = (v[1] / abs(v[1])) * math.sqrt(
                        v[0] ** 2 + v[1] ** 2 - 1)  # transform y velocity so as to maintain the speed
                    v[0] = (
                            2 * paddle.facing - 1)  # note that minimal horiz speed will be lower than we're used
                    # to, where it was 0.95 prior to the  increase by 1.2
